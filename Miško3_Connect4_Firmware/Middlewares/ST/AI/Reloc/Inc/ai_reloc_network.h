...
struct ai_reloc_rt_ctx {
  ...
  uint32_t (*acquire_crc_cb)(void);     /* registered user CB to acquire the CRC IP */
  void (*release_crc_cb)(uint32_t id);  /* registered user CB to release the CRC IP */
};
...
#define AI_RELOC_NETWORK()\
    ...
    { \
      if (_network_rt_ctx.acquire_crc_cb != NULL) \
        return _network_rt_ctx.acquire_crc_cb(); \
      return 0x1; \
    } \
	  \
	  void _ai_platform_release_crc(uint32_t id) \
	  { \
      if (_network_rt_ctx.release_crc_cb != NULL) \
		    _network_rt_ctx.release_crc_cb(id); \
    }
...
/*!
 * @brief   install and create an instance of the network (extended version)
 *          allowing the set the user call-backs to manage the accesses of
 *          the CRC IP.
 *
 * @param   obj             address of the binary object
 * @param   ram_addr        indicate the address of the RAM to install
 ...
 *                           - AI_RELOC_RT_LOAD_MODE_XIP: code is executed in place
 *                           - AI_RELOC_RT_LOAD_MODE_COPY: code is copied in ram before.
 * @param   acquire_crc_cb  callback to acquire the CRC IP
 * @param   release_crc_cb  callback to release the CRC IP
 * @param   hdl             a pointer to a ai_handle object to store the reference
 *                          (opaque object) of the instance.
 *
 * @return an error type/code pair indicating both the error type and code
 ...
 */
AI_API_ENTRY
ai_error ai_rel_network_load_and_create_ext(const void* obj, ai_handle ram_addr,
    ai_size ram_size, uint32_t mode,
    uint32_t (*acquire_crc_cb)(void),
    void (*release_crc_cb)(uint32_t id),
    ai_handle* hdl);
...
/*!
 * @brief   register the user call-backs to manage the CRC IP
 *
 * @param   hdl             network handle (@ref ai_rel_network_load_and_create())
 * @param   acquire_crc_cb  callback to acquire the CRC IP
 * @param   release_crc_cb  callback to release the CRC IP
 *
 * @return false if the hdl is an invalid handle else true
 *
 * Note: to un-register the call-backs, NULL pointers should be passed.
 */
AI_API_ENTRY
ai_bool ai_rel_register_crc_callbacks(ai_handle hdl,
    uint32_t (*acquire_crc_cb)(void),
    void (*release_crc_cb)(uint32_t id));
