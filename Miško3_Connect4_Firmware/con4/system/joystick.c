...
typedef struct
{
	...
	uint16_t 	position_raw_max[NUM_OF_AXES];			// informacija o najmanjšem odklonu osi "joysticka"
	uint16_t 	position_raw_min[NUM_OF_AXES];			// informacija o največjem odklonu osi "joysticka"
...
void JOY_init(ADC_HandleTypeDef *ADC_handle, TIM_HandleTypeDef *timer_handle)
{
	...
	// 2. Nastavitev začetnih vrednosti "joystick" sistemskih spremenljivk
		...
		// Smiselno nastavimo začetne ekstremne vrednosti pozicije "joysticka".
			...
			joystick.position_raw_min[X] = 4095;
			joystick.position_raw_min[Y] = 4095;
			...
			joystick.position_raw_max[X] = 0;
			joystick.position_raw_max[Y] = 0;
	...
	// 4. Poskrbimo za zagon strojne opreme za merjenje signalov "joysticka"
		...
		HAL_ADC_Start_DMA(ADC_handle, (uint32_t *) joystick.position_raw, NUM_OF_AXES);
...
void JOY_calibrate(void)
{
	...
	printf("---------------Calibration. Press the button to stop------------------\n");
...
uint8_t JOY_get_axis_position(joystick_axes_enum_t axis)
{
	...
	if (joystick.position_raw_range[axis] != 0)
		relative_position = (100 * (joystick.position_raw[axis] - joystick.position_raw_min[axis])) / joystick.position_raw_range[axis];

	// Poskrbimo še, da se vrnjena vrednost nahaja na smiselnem intervalu [0..100].
	// Namreč, če funkcija podaja relativno lego osi "joysticka" v procentih, potem
	...
	while (pressed_key != JOY_BTN_NONE)
	{
		...
		switch (pressed_key)
		{
		  case JOY_BTN_FIRE:
			  ...
			  LED_toggle(LED3);	// Spremenimo stanje LEDice 0.
...
void JOY_LED_demo(joystick_axes_enum_t axis)
{
	...
	uint8_t led_num = JOY_get_axis_position(axis) / 13;
printf("%d\n", led_num);




 LEDs_write(1 << led_num); // elegenta resitev
