...
// medpomnilnika.
uint8_t SCI_RX_buffer[SCI_RX_BUF_LEN];			// tabela beatify, ki bo hranila podatke
...
void SCI_send_string_IT(char *str)
{
	...
	if ( i > 0 )
	{
		...
		LL_USART_EnableIT_TXE_TXFNF (SCI.enota);
...
uint8_t SCI_is_data_waiting(void)
{
	...
	return LL_USART_IsActiveFlag_RXNE_RXFNE (SCI.enota);
...
SCI_rtrn_codes_t SCI_read_char(char *c)
{
	...
	if ( SCI_is_data_waiting() )
	{
		...
		*c = LL_USART_ReceiveData8 (SCI.enota);
...
SCI_rtrn_codes_t SCI_read_byte(uint8_t *data)
{
	...
	if (SCI_is_data_waiting())
	{
		...
		*data = LL_USART_ReceiveData8 (SCI.enota);
